...
def stargate(name, proxy, private_key, amount, from_chain, to_chain, from_token, to_token, max_gas, max_value, amountOutMin, slippage=5):
    ...
    try:
        ...
        if amount == 'ALL':
            ...
            amountOutMin = int(amount * 10 ** token_decimals_from)
        else:
            amountIn     = int(amount * 10 ** token_decimals_from)
            amountOutMin = int(amountOutMin * 10 ** token_decimals_from)
    ...
    try: 
        ...
        while True:
            ...
            try:
                ...
                if "Stargate: slippage too high" in str(Ex):
                    logger.warning(f'{name} | {address} | {log_name} | FEE больше, чем установлен в MAX_FEE \n {str(Ex)}')
                    return 'BIG FEE'
